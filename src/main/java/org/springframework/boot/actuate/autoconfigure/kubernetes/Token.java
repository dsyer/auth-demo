...
/**
 * The JSON web token provided with each request that originates from Kubernetes.
 *
...
public class Token {
	...
	public Token(String encoded) {
		...
		if (firstPeriod <= 0 || lastPeriod <= firstPeriod) {
			throw new KubernetesAuthorizationException(Reason.INVALID_TOKEN,
	...
	private Map<String, Object> parseJson(String base64) {
		try {
			...
			throw new KubernetesAuthorizationException(Reason.INVALID_TOKEN, "Token could not be parsed", ex);
		}
	}
	...
	private <T> T getRequired(Map<String, Object> map, String key, Class<T> type) {
		...
		if (value == null) {
			throw new KubernetesAuthorizationException(Reason.INVALID_TOKEN, "Unable to get value from key " + key);
		}
