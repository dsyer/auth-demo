...
import org.springframework.boot.actuate.autoconfigure.kubernetes.servlet.KubernetesWebEndpointServletHandlerMapping.KubernetesWebEndpointServletHandlerMappingRuntimeHints;
 ...
 * Kubernetes specific URLs over HTTP using Spring MVC.
 *
 * @author Madhura Bhave
 * @author Phillip Webb
 * @author Brian Clozel
 */
@ImportRuntimeHints(KubernetesWebEndpointServletHandlerMappingRuntimeHints.class)
class KubernetesWebEndpointServletHandlerMapping extends AbstractWebMvcEndpointHandlerMapping {

	private static final Log logger = LogFactory.getLog(KubernetesWebEndpointServletHandlerMapping.class);

	private final KubernetesSecurityInterceptor securityInterceptor;

	private final EndpointLinksResolver linksResolver;

	private final Collection<ExposableEndpoint<?>> allEndpoints;

	KubernetesWebEndpointServletHandlerMapping(EndpointMapping endpointMapping,
			Collection<ExposableWebEndpoint> endpoints, EndpointMediaTypes endpointMediaTypes,
			CorsConfiguration corsConfiguration, KubernetesSecurityInterceptor securityInterceptor,
	...
	protected LinksHandler getLinksHandler() {
		return new KubernetesLinksHandler();
	}
	...
	class KubernetesLinksHandler implements LinksHandler {

		@Override
		@ResponseBody
		@Reflective
		public Map<String, Map<String, Link>> links(HttpServletRequest request, HttpServletResponse response) {
			SecurityResponse securityResponse = KubernetesWebEndpointServletHandlerMapping.this.securityInterceptor
			...
			Map<String, Link> links = KubernetesWebEndpointServletHandlerMapping.this.linksResolver
	...
	private static class SecureServletWebOperation implements ServletWebOperation {
		...
		SecureServletWebOperation(ServletWebOperation delegate, KubernetesSecurityInterceptor securityInterceptor,
	...
	static class KubernetesWebEndpointServletHandlerMappingRuntimeHints implements RuntimeHintsRegistrar {
		...
		public void registerHints(RuntimeHints hints, ClassLoader classLoader) {
			this.reflectiveRegistrar.registerRuntimeHints(hints, KubernetesLinksHandler.class);
